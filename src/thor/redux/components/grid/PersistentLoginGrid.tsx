...
import { DataObject, PersistentLogin } from '../../../model';

import { useLazyGetPersistentLoginsQuery, useAddPersistentLoginMutation, useUpdatePersistentLoginMutation } from '../../services/PersistentLoginService';
...
import PersistentLoginChart from '../chart/PersistentLoginChart';
import PersistentLoginForm from '../form/PersistentLoginForm';
...
const PersistentLoginGrid: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetPersistentLoginsQuery();
  const [addPersistentLogin] = useAddPersistentLoginMutation();
  const [updatePersistentLogin] = useUpdatePersistentLoginMutation();

  const [data, setData] = useState<PersistentLogin[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<PersistentLogin>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<PersistentLogin | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updatePersistentLogin(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update PersistentLogin:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    ...
    <Tabs key={'PersistentLogin'}>
        
        <Tab
             title="PersistentLogin" 
             eventKey="PersistentLoginC" 
             active={ activeTab == 'PersistentLogin' }
        >
              <PersistentLoginChart /> 
          </Tab>
          
          <Tab title='New PersistentLogin'
                  eventKey="PersistentLoginN"
          >
              <PersistentLoginForm />
          </Tab>
      ...
      <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '100%' } }>
        ...
        <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
          <Modal.Header closeButton>
            <Modal.Title>Editing PersistentLogin</Modal.Title>
          </Modal.Header>
          <Modal.Body>
...
export default PersistentLoginGrid;
