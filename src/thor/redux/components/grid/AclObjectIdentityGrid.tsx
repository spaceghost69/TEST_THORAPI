...
import { DataObject, AclObjectIdentity } from '../../../model';

import { useLazyGetAclObjectIdentitysQuery, useAddAclObjectIdentityMutation, useUpdateAclObjectIdentityMutation } from '../../services/AclObjectIdentityService';
...
import AclObjectIdentityChart from '../chart/AclObjectIdentityChart';
import AclObjectIdentityForm from '../form/AclObjectIdentityForm';
...
const AclObjectIdentityGrid: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetAclObjectIdentitysQuery();
  const [addAclObjectIdentity] = useAddAclObjectIdentityMutation();
  const [updateAclObjectIdentity] = useUpdateAclObjectIdentityMutation();

  const [data, setData] = useState<AclObjectIdentity[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<AclObjectIdentity>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<AclObjectIdentity | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateAclObjectIdentity(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update AclObjectIdentity:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    ...
    <Tabs key={'AclObjectIdentity'}>
        
        <Tab
             title="AclObjectIdentity" 
             eventKey="AclObjectIdentityC" 
             active={ activeTab == 'AclObjectIdentity' }
        >
              <AclObjectIdentityChart /> 
          </Tab>
          
          <Tab title='New AclObjectIdentity'
                  eventKey="AclObjectIdentityN"
          >
              <AclObjectIdentityForm />
          </Tab>
      ...
      <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '100%' } }>
        ...
        <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
          <Modal.Header closeButton>
            <Modal.Title>Editing AclObjectIdentity</Modal.Title>
          </Modal.Header>
          <Modal.Body>
...
export default AclObjectIdentityGrid;
