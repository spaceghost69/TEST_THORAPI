...
import { DataObject, GameWeaponIDX } from '../../../model';

import { useLazyGetGameWeaponIDXsQuery, useAddGameWeaponIDXMutation, useUpdateGameWeaponIDXMutation } from '../../services/GameWeaponIDXService';
...
import GameWeaponIDXChart from '../chart/GameWeaponIDXChart';
import GameWeaponIDXForm from '../form/GameWeaponIDXForm';
...
const GameWeaponIDXGrid: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetGameWeaponIDXsQuery();
  const [addGameWeaponIDX] = useAddGameWeaponIDXMutation();
  const [updateGameWeaponIDX] = useUpdateGameWeaponIDXMutation();

  const [data, setData] = useState<GameWeaponIDX[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<GameWeaponIDX>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<GameWeaponIDX | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateGameWeaponIDX(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update GameWeaponIDX:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    ...
    <Tabs key={'GameWeaponIDX'}>
        
        <Tab
             title="GameWeaponIDX" 
             eventKey="GameWeaponIDXC" 
             active={ activeTab == 'GameWeaponIDX' }
        >
              <GameWeaponIDXChart /> 
          </Tab>
          
          <Tab title='New GameWeaponIDX'
                  eventKey="GameWeaponIDXN"
          >
              <GameWeaponIDXForm />
          </Tab>
      ...
      <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '100%' } }>
        ...
        <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
          <Modal.Header closeButton>
            <Modal.Title>Editing GameWeaponIDX</Modal.Title>
          </Modal.Header>
          <Modal.Body>
...
export default GameWeaponIDXGrid;
