...
import { useLazyGetUserPreferencessQuery, useAddUserPreferencesMutation, useUpdateUserPreferencesMutation } from '../../services/UserPreferencesService';
...
import UserPreferencesChart from '../chart/UserPreferencesChart';
import UserPreferencesForm from '../form/UserPreferencesForm';
...
const UserPreferencesGrid: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetUserPreferencessQuery();
  const [addUserPreferences] = useAddUserPreferencesMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<UserPreferences | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateUserPreferences(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update UserPreferences:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    ...
    <Tabs key={'UserPreferences'}>
        
        <Tab
             title="UserPreferences" 
             eventKey="UserPreferencesC" 
             active={ activeTab == 'UserPreferences' }
        >
              <UserPreferencesChart /> 
          </Tab>
          
          <Tab title='New UserPreferences'
                  eventKey="UserPreferencesN"
          >
              <UserPreferencesForm />
          </Tab>
      ...
      <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '100%' } }>
        ...
        <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
          <Modal.Header closeButton>
            <Modal.Title>Editing UserPreferences</Modal.Title>
          </Modal.Header>
          <Modal.Body>
...
export default UserPreferencesGrid;
