...
import { useLazyGetInvoicesQuery, useAddInvoiceMutation, useUpdateInvoiceMutation } from '../../services/InvoiceService';
...
const InvoiceTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetInvoicesQuery();
  ...
  const [copiedRow, setCopiedRow] = useState<Invoice | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateInvoice(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Invoice</Modal.Title>
        </Modal.Header>
        <Modal.Body>
