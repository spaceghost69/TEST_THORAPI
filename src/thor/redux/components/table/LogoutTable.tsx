...
import { DataObject, Logout } from '../../../model';
import { useLazyGetLogoutsQuery, useAddLogoutMutation, useUpdateLogoutMutation } from '../../services/LogoutService';
...
const LogoutTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetLogoutsQuery();
  const [addLogout] = useAddLogoutMutation();
  const [updateLogout] = useUpdateLogoutMutation();

  const [data, setData] = useState<Logout[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<Logout>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<Logout | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateLogout(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Logout</Modal.Title>
        </Modal.Header>
        <Modal.Body>
