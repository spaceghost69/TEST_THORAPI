...
import { DataObject, Game } from '../../../model';
import { useLazyGetGamesQuery, useAddGameMutation, useUpdateGameMutation } from '../../services/GameService';
...
const GameTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetGamesQuery();
  ...
  const [copiedRow, setCopiedRow] = useState<Game | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateGame(updatedItem).unwrap();
