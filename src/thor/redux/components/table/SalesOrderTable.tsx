...
import { useLazyGetSalesOrdersQuery, useAddSalesOrderMutation, useUpdateSalesOrderMutation } from '../../services/SalesOrderService';
...
const SalesOrderTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetSalesOrdersQuery();
  ...
  const [copiedRow, setCopiedRow] = useState<SalesOrder | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateSalesOrder(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update SalesOrder:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing SalesOrder</Modal.Title>
        </Modal.Header>
        <Modal.Body>
