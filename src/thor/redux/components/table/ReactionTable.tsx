...
import { DataObject, Reaction } from '../../../model';
import { useLazyGetReactionsQuery, useAddReactionMutation, useUpdateReactionMutation } from '../../services/ReactionService';
...
const ReactionTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetReactionsQuery();
  ...
  const [formData, setFormData] = useState<Partial<Reaction>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<Reaction | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateReaction(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Reaction</Modal.Title>
        </Modal.Header>
        <Modal.Body>
