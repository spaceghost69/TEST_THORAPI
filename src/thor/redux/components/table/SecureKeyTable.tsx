...
import { DataObject, SecureKey } from '../../../model';
import { useLazyGetSecureKeysQuery, useAddSecureKeyMutation, useUpdateSecureKeyMutation } from '../../services/SecureKeyService';
...
const SecureKeyTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetSecureKeysQuery();
  const [addSecureKey] = useAddSecureKeyMutation();
  const [updateSecureKey] = useUpdateSecureKeyMutation();

  const [data, setData] = useState<SecureKey[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<SecureKey>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SecureKey | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateSecureKey(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update SecureKey:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing SecureKey</Modal.Title>
        </Modal.Header>
        <Modal.Body>
