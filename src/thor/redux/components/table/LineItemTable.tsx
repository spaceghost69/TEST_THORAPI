...
import { DataObject, LineItem } from '../../../model';
import { useLazyGetLineItemsQuery, useAddLineItemMutation, useUpdateLineItemMutation } from '../../services/LineItemService';
...
const LineItemTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetLineItemsQuery();
  ...
  const [copiedRow, setCopiedRow] = useState<LineItem | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateLineItem(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update LineItem:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing LineItem</Modal.Title>
        </Modal.Header>
        <Modal.Body>
