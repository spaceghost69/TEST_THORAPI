...
import { DataObject, Opportunity } from '../../../model';
import { useLazyGetOpportunitysQuery, useAddOpportunityMutation, useUpdateOpportunityMutation } from '../../services/OpportunityService';
...
const OpportunityTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetOpportunitysQuery();
  const [addOpportunity] = useAddOpportunityMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<Opportunity | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateOpportunity(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Opportunity</Modal.Title>
        </Modal.Header>
        <Modal.Body>
