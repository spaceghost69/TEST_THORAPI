...
import { DataObject, Task } from '../../../model';
import { useLazyGetTasksQuery, useAddTaskMutation, useUpdateTaskMutation } from '../../services/TaskService';
...
const TaskTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetTasksQuery();
  ...
  const [copiedRow, setCopiedRow] = useState<Task | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateTask(updatedItem).unwrap();
