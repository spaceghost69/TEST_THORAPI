...
import { useLazyGetAclSidsQuery, useAddAclSidMutation, useUpdateAclSidMutation } from '../../services/AclSidService';
...
const AclSidTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetAclSidsQuery();
  const [addAclSid] = useAddAclSidMutation();
  const [updateAclSid] = useUpdateAclSidMutation();

  const [data, setData] = useState<AclSid[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<AclSid>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<AclSid | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateAclSid(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update AclSid:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing AclSid</Modal.Title>
        </Modal.Header>
        <Modal.Body>
