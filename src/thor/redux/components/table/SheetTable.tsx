...
import { DataObject, Sheet } from '../../../model';
import { useLazyGetSheetsQuery, useAddSheetMutation, useUpdateSheetMutation } from '../../services/SheetService';
...
const SheetTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetSheetsQuery();
  ...
  const [formData, setFormData] = useState<Partial<Sheet>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<Sheet | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateSheet(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Sheet</Modal.Title>
        </Modal.Header>
        <Modal.Body>
