...
import { DataObject, StrategicPriority } from '../../../model';
import { useLazyGetStrategicPrioritysQuery, useAddStrategicPriorityMutation, useUpdateStrategicPriorityMutation } from '../../services/StrategicPriorityService';
...
const StrategicPriorityTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetStrategicPrioritysQuery();
  const [addStrategicPriority] = useAddStrategicPriorityMutation();
  const [updateStrategicPriority] = useUpdateStrategicPriorityMutation();

  const [data, setData] = useState<StrategicPriority[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<StrategicPriority>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<StrategicPriority | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateStrategicPriority(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update StrategicPriority:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing StrategicPriority</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default StrategicPriorityTable;
