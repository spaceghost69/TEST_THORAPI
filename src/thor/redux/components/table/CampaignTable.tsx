...
import { DataObject, Campaign } from '../../../model';
import { useLazyGetCampaignsQuery, useAddCampaignMutation, useUpdateCampaignMutation } from '../../services/CampaignService';
...
const CampaignTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetCampaignsQuery();
  ...
  const [copiedRow, setCopiedRow] = useState<Campaign | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateCampaign(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Campaign</Modal.Title>
        </Modal.Header>
        <Modal.Body>
