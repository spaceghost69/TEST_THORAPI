...
import { DataObject, Application } from '../../../model';
import { useLazyGetApplicationsQuery, useAddApplicationMutation, useUpdateApplicationMutation } from '../../services/ApplicationService';
...
const ApplicationTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetApplicationsQuery();
  ...
  const [copiedRow, setCopiedRow] = useState<Application | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateApplication(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Application</Modal.Title>
        </Modal.Header>
        <Modal.Body>
