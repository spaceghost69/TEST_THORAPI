...
import { DataObject, Authority } from '../../../model';
import { useLazyGetAuthoritysQuery, useAddAuthorityMutation, useUpdateAuthorityMutation } from '../../services/AuthorityService';
...
const AuthorityTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetAuthoritysQuery();
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateAuthority(updatedItem).unwrap();
