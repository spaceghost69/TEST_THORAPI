...
import { DataObject, NamedRange } from '../../../model';
import { useLazyGetNamedRangesQuery, useAddNamedRangeMutation, useUpdateNamedRangeMutation } from '../../services/NamedRangeService';
...
const NamedRangeTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetNamedRangesQuery();
  const [addNamedRange] = useAddNamedRangeMutation();
  const [updateNamedRange] = useUpdateNamedRangeMutation();

  const [data, setData] = useState<NamedRange[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<NamedRange>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<NamedRange | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateNamedRange(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update NamedRange:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing NamedRange</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default NamedRangeTable;
