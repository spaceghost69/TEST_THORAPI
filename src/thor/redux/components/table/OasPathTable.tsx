...
import { DataObject, OasPath } from '../../../model';
import { useLazyGetOasPathsQuery, useAddOasPathMutation, useUpdateOasPathMutation } from '../../services/OasPathService';
...
const OasPathTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetOasPathsQuery();
  const [addOasPath] = useAddOasPathMutation();
  const [updateOasPath] = useUpdateOasPathMutation();

  const [data, setData] = useState<OasPath[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<OasPath>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<OasPath | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateOasPath(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update OasPath:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing OasPath</Modal.Title>
        </Modal.Header>
        <Modal.Body>
