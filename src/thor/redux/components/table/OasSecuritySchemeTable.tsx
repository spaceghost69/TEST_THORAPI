...
import { DataObject, OasSecurityScheme } from '../../../model';
import { useLazyGetOasSecuritySchemesQuery, useAddOasSecuritySchemeMutation, useUpdateOasSecuritySchemeMutation } from '../../services/OasSecuritySchemeService';
...
const OasSecuritySchemeTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetOasSecuritySchemesQuery();
  const [addOasSecurityScheme] = useAddOasSecuritySchemeMutation();
  const [updateOasSecurityScheme] = useUpdateOasSecuritySchemeMutation();

  const [data, setData] = useState<OasSecurityScheme[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<OasSecurityScheme>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<OasSecurityScheme | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateOasSecurityScheme(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update OasSecurityScheme:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing OasSecurityScheme</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default OasSecuritySchemeTable;
