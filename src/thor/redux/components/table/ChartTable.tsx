...
import { DataObject, Chart } from '../../../model';
import { useLazyGetChartsQuery, useAddChartMutation, useUpdateChartMutation } from '../../services/ChartService';
...
const ChartTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetChartsQuery();
  ...
  const [formData, setFormData] = useState<Partial<Chart>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<Chart | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateChart(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Chart</Modal.Title>
        </Modal.Header>
        <Modal.Body>
