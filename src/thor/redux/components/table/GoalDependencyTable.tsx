...
import { DataObject, GoalDependency } from '../../../model';
import { useLazyGetGoalDependencysQuery, useAddGoalDependencyMutation, useUpdateGoalDependencyMutation } from '../../services/GoalDependencyService';
...
const GoalDependencyTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetGoalDependencysQuery();
  const [addGoalDependency] = useAddGoalDependencyMutation();
  const [updateGoalDependency] = useUpdateGoalDependencyMutation();

  const [data, setData] = useState<GoalDependency[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<GoalDependency>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<GoalDependency | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateGoalDependency(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update GoalDependency:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing GoalDependency</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default GoalDependencyTable;
