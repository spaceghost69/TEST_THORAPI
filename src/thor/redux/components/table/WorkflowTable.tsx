...
import { DataObject, Workflow } from '../../../model';
import { useLazyGetWorkflowsQuery, useAddWorkflowMutation, useUpdateWorkflowMutation } from '../../services/WorkflowService';
...
const WorkflowTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetWorkflowsQuery();
  ...
  const [copiedRow, setCopiedRow] = useState<Workflow | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateWorkflow(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Workflow</Modal.Title>
        </Modal.Header>
        <Modal.Body>
