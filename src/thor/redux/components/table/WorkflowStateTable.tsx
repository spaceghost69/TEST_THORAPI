...
import { useLazyGetWorkflowStatesQuery, useAddWorkflowStateMutation, useUpdateWorkflowStateMutation } from '../../services/WorkflowStateService';
...
const WorkflowStateTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetWorkflowStatesQuery();
  const [addWorkflowState] = useAddWorkflowStateMutation();
  const [updateWorkflowState] = useUpdateWorkflowStateMutation();

  const [data, setData] = useState<WorkflowState[]>([]);
  ...
  const [copiedRow, setCopiedRow] = useState<WorkflowState | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateWorkflowState(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update WorkflowState:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing WorkflowState</Modal.Title>
        </Modal.Header>
        <Modal.Body>
