...
import { useLazyGetOrganizationsQuery, useAddOrganizationMutation, useUpdateOrganizationMutation } from '../../services/OrganizationService';
...
const OrganizationTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetOrganizationsQuery();
  ...
  const [copiedRow, setCopiedRow] = useState<Organization | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateOrganization(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Organization</Modal.Title>
        </Modal.Header>
        <Modal.Body>
