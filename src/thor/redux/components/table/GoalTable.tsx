...
import { DataObject, Goal } from '../../../model';
import { useLazyGetGoalsQuery, useAddGoalMutation, useUpdateGoalMutation } from '../../services/GoalService';
...
const GoalTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetGoalsQuery();
  ...
  const [copiedRow, setCopiedRow] = useState<Goal | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateGoal(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Goal</Modal.Title>
        </Modal.Header>
        <Modal.Body>
