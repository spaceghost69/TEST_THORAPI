...
import { DataObject, Attack } from '../../../model';
import { useLazyGetAttacksQuery, useAddAttackMutation, useUpdateAttackMutation } from '../../services/AttackService';
...
const AttackTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetAttacksQuery();
  const [addAttack] = useAddAttackMutation();
  const [updateAttack] = useUpdateAttackMutation();

  const [data, setData] = useState<Attack[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<Attack>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<Attack | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateAttack(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Attack</Modal.Title>
        </Modal.Header>
        <Modal.Body>
