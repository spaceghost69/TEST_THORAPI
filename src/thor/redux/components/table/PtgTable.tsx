...
import { DataObject, Ptg } from '../../../model';
import { useLazyGetPtgsQuery, useAddPtgMutation, useUpdatePtgMutation } from '../../services/PtgService';
...
const PtgTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetPtgsQuery();
  const [addPtg] = useAddPtgMutation();
  const [updatePtg] = useUpdatePtgMutation();

  const [data, setData] = useState<Ptg[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<Ptg>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<Ptg | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updatePtg(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update Ptg:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Ptg</Modal.Title>
        </Modal.Header>
        <Modal.Body>
