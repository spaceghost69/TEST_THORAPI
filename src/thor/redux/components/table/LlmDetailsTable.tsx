...
import { DataObject, LlmDetails } from '../../../model';
import { useLazyGetLlmDetailssQuery, useAddLlmDetailsMutation, useUpdateLlmDetailsMutation } from '../../services/LlmDetailsService';
...
const LlmDetailsTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetLlmDetailssQuery();
  const [addLlmDetails] = useAddLlmDetailsMutation();
  const [updateLlmDetails] = useUpdateLlmDetailsMutation();

  const [data, setData] = useState<LlmDetails[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<LlmDetails>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<LlmDetails | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateLlmDetails(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update LlmDetails:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing LlmDetails</Modal.Title>
        </Modal.Header>
        <Modal.Body>
