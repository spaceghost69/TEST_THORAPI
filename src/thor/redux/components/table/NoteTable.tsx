...
import { DataObject, Note } from '../../../model';
import { useLazyGetNotesQuery, useAddNoteMutation, useUpdateNoteMutation } from '../../services/NoteService';
...
const NoteTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetNotesQuery();
  ...
  const [copiedRow, setCopiedRow] = useState<Note | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateNote(updatedItem).unwrap();
