...
import { useLazyGetOasEnumsQuery, useAddOasEnumMutation, useUpdateOasEnumMutation } from '../../services/OasEnumService';
...
const OasEnumTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetOasEnumsQuery();
  const [addOasEnum] = useAddOasEnumMutation();
  const [updateOasEnum] = useUpdateOasEnumMutation();

  const [data, setData] = useState<OasEnum[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<OasEnum>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<OasEnum | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateOasEnum(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update OasEnum:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing OasEnum</Modal.Title>
        </Modal.Header>
        <Modal.Body>
