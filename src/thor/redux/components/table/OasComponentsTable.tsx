...
import { useLazyGetOasComponentssQuery, useAddOasComponentsMutation, useUpdateOasComponentsMutation } from '../../services/OasComponentsService';
...
const OasComponentsTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetOasComponentssQuery();
  const [addOasComponents] = useAddOasComponentsMutation();
  const [updateOasComponents] = useUpdateOasComponentsMutation();

  const [data, setData] = useState<OasComponents[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<OasComponents>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<OasComponents | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateOasComponents(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update OasComponents:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing OasComponents</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default OasComponentsTable;
