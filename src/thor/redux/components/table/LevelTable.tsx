...
import { DataObject, Level } from '../../../model';
import { useLazyGetLevelsQuery, useAddLevelMutation, useUpdateLevelMutation } from '../../services/LevelService';
...
const LevelTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetLevelsQuery();
  ...
  const [copiedRow, setCopiedRow] = useState<Level | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateLevel(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Level</Modal.Title>
        </Modal.Header>
        <Modal.Body>
