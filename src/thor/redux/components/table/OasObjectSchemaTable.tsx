...
import { DataObject, OasObjectSchema } from '../../../model';
import { useLazyGetOasObjectSchemasQuery, useAddOasObjectSchemaMutation, useUpdateOasObjectSchemaMutation } from '../../services/OasObjectSchemaService';
...
const OasObjectSchemaTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetOasObjectSchemasQuery();
  const [addOasObjectSchema] = useAddOasObjectSchemaMutation();
  const [updateOasObjectSchema] = useUpdateOasObjectSchemaMutation();

  const [data, setData] = useState<OasObjectSchema[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<OasObjectSchema>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<OasObjectSchema | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateOasObjectSchema(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update OasObjectSchema:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing OasObjectSchema</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default OasObjectSchemaTable;
