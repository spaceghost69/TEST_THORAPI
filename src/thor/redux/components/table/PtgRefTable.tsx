...
import { DataObject, PtgRef } from '../../../model';
import { useLazyGetPtgRefsQuery, useAddPtgRefMutation, useUpdatePtgRefMutation } from '../../services/PtgRefService';
...
const PtgRefTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetPtgRefsQuery();
  const [addPtgRef] = useAddPtgRefMutation();
  const [updatePtgRef] = useUpdatePtgRefMutation();

  const [data, setData] = useState<PtgRef[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<PtgRef>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<PtgRef | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updatePtgRef(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update PtgRef:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing PtgRef</Modal.Title>
        </Modal.Header>
        <Modal.Body>
