...
import { DataObject, Weapon } from '../../../model';
import { useLazyGetWeaponsQuery, useAddWeaponMutation, useUpdateWeaponMutation } from '../../services/WeaponService';
...
const WeaponTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetWeaponsQuery();
  const [addWeapon] = useAddWeaponMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<Weapon | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateWeapon(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Weapon</Modal.Title>
        </Modal.Header>
        <Modal.Body>
