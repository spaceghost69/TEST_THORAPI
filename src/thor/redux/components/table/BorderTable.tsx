...
import { DataObject, Border } from '../../../model';
import { useLazyGetBordersQuery, useAddBorderMutation, useUpdateBorderMutation } from '../../services/BorderService';
...
const BorderTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetBordersQuery();
  const [addBorder] = useAddBorderMutation();
  const [updateBorder] = useUpdateBorderMutation();

  const [data, setData] = useState<Border[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<Border>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<Border | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateBorder(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Border</Modal.Title>
        </Modal.Header>
        <Modal.Body>
