...
import { DataObject, Address } from '../../../model';
import { useLazyGetAddresssQuery, useAddAddressMutation, useUpdateAddressMutation } from '../../services/AddressService';
...
const AddressTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetAddresssQuery();
  ...
  const [copiedRow, setCopiedRow] = useState<Address | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateAddress(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Address</Modal.Title>
        </Modal.Header>
        <Modal.Body>
