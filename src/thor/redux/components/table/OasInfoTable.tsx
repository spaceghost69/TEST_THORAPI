...
import { useLazyGetOasInfosQuery, useAddOasInfoMutation, useUpdateOasInfoMutation } from '../../services/OasInfoService';
...
const OasInfoTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetOasInfosQuery();
  const [addOasInfo] = useAddOasInfoMutation();
  const [updateOasInfo] = useUpdateOasInfoMutation();

  const [data, setData] = useState<OasInfo[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<OasInfo>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<OasInfo | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateOasInfo(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update OasInfo:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing OasInfo</Modal.Title>
        </Modal.Header>
        <Modal.Body>
