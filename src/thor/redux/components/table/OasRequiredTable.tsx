...
import { DataObject, OasRequired } from '../../../model';
import { useLazyGetOasRequiredsQuery, useAddOasRequiredMutation, useUpdateOasRequiredMutation } from '../../services/OasRequiredService';
...
const OasRequiredTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetOasRequiredsQuery();
  const [addOasRequired] = useAddOasRequiredMutation();
  const [updateOasRequired] = useUpdateOasRequiredMutation();

  const [data, setData] = useState<OasRequired[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<OasRequired>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<OasRequired | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateOasRequired(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update OasRequired:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing OasRequired</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default OasRequiredTable;
