...
import { DataObject, AclEntry } from '../../../model';
import { useLazyGetAclEntrysQuery, useAddAclEntryMutation, useUpdateAclEntryMutation } from '../../services/AclEntryService';
...
const AclEntryTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetAclEntrysQuery();
  const [addAclEntry] = useAddAclEntryMutation();
  const [updateAclEntry] = useUpdateAclEntryMutation();

  const [data, setData] = useState<AclEntry[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<AclEntry>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<AclEntry | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateAclEntry(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update AclEntry:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing AclEntry</Modal.Title>
        </Modal.Header>
        <Modal.Body>
