...
import { DataObject, OasResponse } from '../../../model';
import { useLazyGetOasResponsesQuery, useAddOasResponseMutation, useUpdateOasResponseMutation } from '../../services/OasResponseService';
...
const OasResponseTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetOasResponsesQuery();
  const [addOasResponse] = useAddOasResponseMutation();
  const [updateOasResponse] = useUpdateOasResponseMutation();

  const [data, setData] = useState<OasResponse[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<OasResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<OasResponse | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateOasResponse(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update OasResponse:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing OasResponse</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default OasResponseTable;
