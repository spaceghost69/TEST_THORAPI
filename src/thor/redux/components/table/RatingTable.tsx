...
import { useLazyGetRatingsQuery, useAddRatingMutation, useUpdateRatingMutation } from '../../services/RatingService';
...
const RatingTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetRatingsQuery();
  ...
  const [copiedRow, setCopiedRow] = useState<Rating | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateRating(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Rating</Modal.Title>
        </Modal.Header>
        <Modal.Body>
