...
import { DataObject, Workbook } from '../../../model';
import { useLazyGetWorkbooksQuery, useAddWorkbookMutation, useUpdateWorkbookMutation } from '../../services/WorkbookService';
...
const WorkbookTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetWorkbooksQuery();
  const [addWorkbook] = useAddWorkbookMutation();
  const [updateWorkbook] = useUpdateWorkbookMutation();

  const [data, setData] = useState<Workbook[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<Workbook>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<Workbook | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateWorkbook(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Workbook</Modal.Title>
        </Modal.Header>
        <Modal.Body>
