...
import { DataObject, SheetColumn } from '../../../model';
import { useLazyGetSheetColumnsQuery, useAddSheetColumnMutation, useUpdateSheetColumnMutation } from '../../services/SheetColumnService';
...
const SheetColumnTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetSheetColumnsQuery();
  const [addSheetColumn] = useAddSheetColumnMutation();
  const [updateSheetColumn] = useUpdateSheetColumnMutation();

  const [data, setData] = useState<SheetColumn[]>([]);
  ...
  const [copiedRow, setCopiedRow] = useState<SheetColumn | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateSheetColumn(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update SheetColumn:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing SheetColumn</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default SheetColumnTable;
