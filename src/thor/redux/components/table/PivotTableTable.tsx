...
import { DataObject, PivotTable } from '../../../model';
import { useLazyGetPivotTablesQuery, useAddPivotTableMutation, useUpdatePivotTableMutation } from '../../services/PivotTableService';
...
const PivotTableTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetPivotTablesQuery();
  const [addPivotTable] = useAddPivotTableMutation();
  const [updatePivotTable] = useUpdatePivotTableMutation();

  const [data, setData] = useState<PivotTable[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<PivotTable>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<PivotTable | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updatePivotTable(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update PivotTable:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing PivotTable</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default PivotTableTable;
