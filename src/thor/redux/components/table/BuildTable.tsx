...
import { DataObject, Build } from '../../../model';
import { useLazyGetBuildsQuery, useAddBuildMutation, useUpdateBuildMutation } from '../../services/BuildService';
...
const BuildTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetBuildsQuery();
  const [addBuild] = useAddBuildMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<Build | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateBuild(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Build</Modal.Title>
        </Modal.Header>
        <Modal.Body>
