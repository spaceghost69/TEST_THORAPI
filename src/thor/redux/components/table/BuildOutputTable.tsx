...
import { useLazyGetBuildOutputsQuery, useAddBuildOutputMutation, useUpdateBuildOutputMutation } from '../../services/BuildOutputService';
...
const BuildOutputTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetBuildOutputsQuery();
  const [addBuildOutput] = useAddBuildOutputMutation();
  const [updateBuildOutput] = useUpdateBuildOutputMutation();

  const [data, setData] = useState<BuildOutput[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<BuildOutput>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<BuildOutput | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateBuildOutput(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update BuildOutput:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing BuildOutput</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default BuildOutputTable;
