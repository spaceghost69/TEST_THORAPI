...
import { useLazyGetBackupConfigsQuery, useAddBackupConfigMutation, useUpdateBackupConfigMutation } from '../../services/BackupConfigService';
...
const BackupConfigTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetBackupConfigsQuery();
  const [addBackupConfig] = useAddBackupConfigMutation();
  const [updateBackupConfig] = useUpdateBackupConfigMutation();

  const [data, setData] = useState<BackupConfig[]>([]);
  ...
  const [copiedRow, setCopiedRow] = useState<BackupConfig | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateBackupConfig(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update BackupConfig:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing BackupConfig</Modal.Title>
        </Modal.Header>
        <Modal.Body>
