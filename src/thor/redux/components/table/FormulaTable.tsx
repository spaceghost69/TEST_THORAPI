...
import { DataObject, Formula } from '../../../model';
import { useLazyGetFormulasQuery, useAddFormulaMutation, useUpdateFormulaMutation } from '../../services/FormulaService';
...
const FormulaTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetFormulasQuery();
  ...
  const [copiedRow, setCopiedRow] = useState<Formula | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateFormula(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Formula</Modal.Title>
        </Modal.Header>
        <Modal.Body>
