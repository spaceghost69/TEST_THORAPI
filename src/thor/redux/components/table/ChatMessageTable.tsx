...
import { DataObject, ChatMessage } from '../../../model';
import { useLazyGetChatMessagesQuery, useAddChatMessageMutation, useUpdateChatMessageMutation } from '../../services/ChatMessageService';
...
const ChatMessageTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetChatMessagesQuery();
  ...
  const [formData, setFormData] = useState<Partial<ChatMessage>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ChatMessage | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateChatMessage(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update ChatMessage:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing ChatMessage</Modal.Title>
        </Modal.Header>
        <Modal.Body>
