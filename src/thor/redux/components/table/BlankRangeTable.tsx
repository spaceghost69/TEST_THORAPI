...
import { DataObject, BlankRange } from '../../../model';
import { useLazyGetBlankRangesQuery, useAddBlankRangeMutation, useUpdateBlankRangeMutation } from '../../services/BlankRangeService';
...
const BlankRangeTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetBlankRangesQuery();
  const [addBlankRange] = useAddBlankRangeMutation();
  const [updateBlankRange] = useUpdateBlankRangeMutation();

  const [data, setData] = useState<BlankRange[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<BlankRange>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<BlankRange | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateBlankRange(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update BlankRange:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing BlankRange</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default BlankRangeTable;
