...
import { DataObject, Solution } from '../../../model';
import { useLazyGetSolutionsQuery, useAddSolutionMutation, useUpdateSolutionMutation } from '../../services/SolutionService';
...
const SolutionTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetSolutionsQuery();
  ...
  const [copiedRow, setCopiedRow] = useState<Solution | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateSolution(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Solution</Modal.Title>
        </Modal.Header>
        <Modal.Body>
