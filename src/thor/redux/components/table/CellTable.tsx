...
import { DataObject, Cell } from '../../../model';
import { useLazyGetCellsQuery, useAddCellMutation, useUpdateCellMutation } from '../../services/CellService';
...
const CellTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetCellsQuery();
  ...
  const [copiedRow, setCopiedRow] = useState<Cell | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateCell(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Cell</Modal.Title>
        </Modal.Header>
        <Modal.Body>
