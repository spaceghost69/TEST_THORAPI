...
import { DataObject, ContentData } from '../../../model';
import { useLazyGetContentDatasQuery, useAddContentDataMutation, useUpdateContentDataMutation } from '../../services/ContentDataService';
...
const ContentDataTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetContentDatasQuery();
  ...
  const [copiedRow, setCopiedRow] = useState<ContentData | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateContentData(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update ContentData:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing ContentData</Modal.Title>
        </Modal.Header>
        <Modal.Body>
