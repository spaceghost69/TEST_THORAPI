...
import { useLazyGetDependsQuery, useAddDependMutation, useUpdateDependMutation } from '../../services/DependService';
...
const DependTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetDependsQuery();
  const [addDepend] = useAddDependMutation();
  const [updateDepend] = useUpdateDependMutation();

  const [data, setData] = useState<Depend[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<Depend>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<Depend | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateDepend(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update Depend:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Depend</Modal.Title>
        </Modal.Header>
        <Modal.Body>
