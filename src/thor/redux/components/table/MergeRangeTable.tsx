...
import { DataObject, MergeRange } from '../../../model';
import { useLazyGetMergeRangesQuery, useAddMergeRangeMutation, useUpdateMergeRangeMutation } from '../../services/MergeRangeService';
...
const MergeRangeTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetMergeRangesQuery();
  const [addMergeRange] = useAddMergeRangeMutation();
  const [updateMergeRange] = useUpdateMergeRangeMutation();

  const [data, setData] = useState<MergeRange[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<MergeRange>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<MergeRange | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateMergeRange(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update MergeRange:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing MergeRange</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default MergeRangeTable;
