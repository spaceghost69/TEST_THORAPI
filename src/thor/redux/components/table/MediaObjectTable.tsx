...
import { DataObject, MediaObject } from '../../../model';
import { useLazyGetMediaObjectsQuery, useAddMediaObjectMutation, useUpdateMediaObjectMutation } from '../../services/MediaObjectService';
...
const MediaObjectTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetMediaObjectsQuery();
  const [addMediaObject] = useAddMediaObjectMutation();
  const [updateMediaObject] = useUpdateMediaObjectMutation();

  const [data, setData] = useState<MediaObject[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<MediaObject>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<MediaObject | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateMediaObject(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update MediaObject:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing MediaObject</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default MediaObjectTable;
