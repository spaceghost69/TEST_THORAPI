...
import { DataObject, ChatResponse } from '../../../model';
import { useLazyGetChatResponsesQuery, useAddChatResponseMutation, useUpdateChatResponseMutation } from '../../services/ChatResponseService';
...
const ChatResponseTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetChatResponsesQuery();
  const [addChatResponse] = useAddChatResponseMutation();
  const [updateChatResponse] = useUpdateChatResponseMutation();

  const [data, setData] = useState<ChatResponse[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ChatResponse>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ChatResponse | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateChatResponse(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update ChatResponse:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing ChatResponse</Modal.Title>
        </Modal.Header>
        <Modal.Body>
