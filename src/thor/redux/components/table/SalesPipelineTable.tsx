...
import { DataObject, SalesPipeline } from '../../../model';
import { useLazyGetSalesPipelinesQuery, useAddSalesPipelineMutation, useUpdateSalesPipelineMutation } from '../../services/SalesPipelineService';
...
const SalesPipelineTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetSalesPipelinesQuery();
  const [addSalesPipeline] = useAddSalesPipelineMutation();
  const [updateSalesPipeline] = useUpdateSalesPipelineMutation();

  const [data, setData] = useState<SalesPipeline[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<SalesPipeline>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<SalesPipeline | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateSalesPipeline(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update SalesPipeline:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing SalesPipeline</Modal.Title>
        </Modal.Header>
        <Modal.Body>
