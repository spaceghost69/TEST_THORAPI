...
import { DataObject, Format } from '../../../model';
import { useLazyGetFormatsQuery, useAddFormatMutation, useUpdateFormatMutation } from '../../services/FormatService';
...
const FormatTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetFormatsQuery();
  ...
  const [formData, setFormData] = useState<Partial<Format>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<Format | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateFormat(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Format</Modal.Title>
        </Modal.Header>
        <Modal.Body>
