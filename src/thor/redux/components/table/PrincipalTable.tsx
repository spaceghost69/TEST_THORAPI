...
import { useLazyGetPrincipalsQuery, useAddPrincipalMutation, useUpdatePrincipalMutation } from '../../services/PrincipalService';
...
const PrincipalTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetPrincipalsQuery();
  const [addPrincipal] = useAddPrincipalMutation();
  const [updatePrincipal] = useUpdatePrincipalMutation();

  const [data, setData] = useState<Principal[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<Principal>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<Principal | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updatePrincipal(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Principal</Modal.Title>
        </Modal.Header>
        <Modal.Body>
