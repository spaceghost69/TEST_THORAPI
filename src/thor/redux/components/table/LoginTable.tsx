...
import { DataObject, Login } from '../../../model';
import { useLazyGetLoginsQuery, useAddLoginMutation, useUpdateLoginMutation } from '../../services/LoginService';
...
const LoginTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetLoginsQuery();
  ...
  const [copiedRow, setCopiedRow] = useState<Login | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateLogin(updatedItem).unwrap();
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing Login</Modal.Title>
        </Modal.Header>
        <Modal.Body>
