...
import { DataObject, EventLog } from '../../../model';
import { useLazyGetEventLogsQuery, useAddEventLogMutation, useUpdateEventLogMutation } from '../../services/EventLogService';
...
const EventLogTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetEventLogsQuery();
  const [addEventLog] = useAddEventLogMutation();
  const [updateEventLog] = useUpdateEventLogMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<EventLog | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateEventLog(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update EventLog:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing EventLog</Modal.Title>
        </Modal.Header>
        <Modal.Body>
