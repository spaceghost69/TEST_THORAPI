...
import { useLazyGetAclClasssQuery, useAddAclClassMutation, useUpdateAclClassMutation } from '../../services/AclClassService';
...
const AclClassTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetAclClasssQuery();
  const [addAclClass] = useAddAclClassMutation();
  const [updateAclClass] = useUpdateAclClassMutation();

  const [data, setData] = useState<AclClass[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<AclClass>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<AclClass | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateAclClass(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update AclClass:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing AclClass</Modal.Title>
        </Modal.Header>
        <Modal.Body>
