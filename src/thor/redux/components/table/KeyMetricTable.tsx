...
import { DataObject, KeyMetric } from '../../../model';
import { useLazyGetKeyMetricsQuery, useAddKeyMetricMutation, useUpdateKeyMetricMutation } from '../../services/KeyMetricService';
...
const KeyMetricTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetKeyMetricsQuery();
  const [addKeyMetric] = useAddKeyMetricMutation();
  const [updateKeyMetric] = useUpdateKeyMetricMutation();

  const [data, setData] = useState<KeyMetric[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<KeyMetric>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<KeyMetric | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateKeyMetric(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update KeyMetric:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing KeyMetric</Modal.Title>
        </Modal.Header>
        <Modal.Body>
