...
import { Form as BSForm, Col, Row, Container, Spinner, Modal, Button } from 'react-bootstrap';
...
import { DataObject, IntegrationAccount } from '../../../model';
import { useLazyGetIntegrationAccountsQuery, useAddIntegrationAccountMutation, useUpdateIntegrationAccountMutation } from '../../services/IntegrationAccountService';
...
const IntegrationAccountTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetIntegrationAccountsQuery();
  const [addIntegrationAccount] = useAddIntegrationAccountMutation();
  const [updateIntegrationAccount] = useUpdateIntegrationAccountMutation();

  const [data, setData] = useState<IntegrationAccount[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<IntegrationAccount>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<IntegrationAccount | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateIntegrationAccount(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update IntegrationAccount:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing IntegrationAccount</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default IntegrationAccountTable;
