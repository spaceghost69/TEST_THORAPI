...
import { DataObject, OasParameter } from '../../../model';
import { useLazyGetOasParametersQuery, useAddOasParameterMutation, useUpdateOasParameterMutation } from '../../services/OasParameterService';
...
const OasParameterTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetOasParametersQuery();
  const [addOasParameter] = useAddOasParameterMutation();
  const [updateOasParameter] = useUpdateOasParameterMutation();

  const [data, setData] = useState<OasParameter[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<OasParameter>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<OasParameter | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateOasParameter(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update OasParameter:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing OasParameter</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default OasParameterTable;
