...
import { DataObject, ThorUXMeta } from '../../../model';
import { useLazyGetThorUXMetasQuery, useAddThorUXMetaMutation, useUpdateThorUXMetaMutation } from '../../services/ThorUXMetaService';
...
const ThorUXMetaTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetThorUXMetasQuery();
  const [addThorUXMeta] = useAddThorUXMetaMutation();
  const [updateThorUXMeta] = useUpdateThorUXMetaMutation();

  const [data, setData] = useState<ThorUXMeta[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ThorUXMeta>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ThorUXMeta | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateThorUXMeta(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update ThorUXMeta:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing ThorUXMeta</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default ThorUXMetaTable;
