...
import { useLazyGetOasOpenAPISpecsQuery, useAddOasOpenAPISpecMutation, useUpdateOasOpenAPISpecMutation } from '../../services/OasOpenAPISpecService';
...
const OasOpenAPISpecTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetOasOpenAPISpecsQuery();
  const [addOasOpenAPISpec] = useAddOasOpenAPISpecMutation();
  const [updateOasOpenAPISpec] = useUpdateOasOpenAPISpecMutation();

  const [data, setData] = useState<OasOpenAPISpec[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<OasOpenAPISpec>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<OasOpenAPISpec | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateOasOpenAPISpec(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update OasOpenAPISpec:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing OasOpenAPISpec</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default OasOpenAPISpecTable;
