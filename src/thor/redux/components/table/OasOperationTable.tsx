...
import { DataObject, OasOperation } from '../../../model';
import { useLazyGetOasOperationsQuery, useAddOasOperationMutation, useUpdateOasOperationMutation } from '../../services/OasOperationService';
...
const OasOperationTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetOasOperationsQuery();
  const [addOasOperation] = useAddOasOperationMutation();
  const [updateOasOperation] = useUpdateOasOperationMutation();

  const [data, setData] = useState<OasOperation[]>([]);
