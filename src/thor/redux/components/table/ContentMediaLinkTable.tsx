...
import { useLazyGetContentMediaLinksQuery, useAddContentMediaLinkMutation, useUpdateContentMediaLinkMutation } from '../../services/ContentMediaLinkService';
...
const ContentMediaLinkTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetContentMediaLinksQuery();
  const [addContentMediaLink] = useAddContentMediaLinkMutation();
  const [updateContentMediaLink] = useUpdateContentMediaLinkMutation();

  const [data, setData] = useState<ContentMediaLink[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ContentMediaLink>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ContentMediaLink | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateContentMediaLink(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update ContentMediaLink:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing ContentMediaLink</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default ContentMediaLinkTable;
