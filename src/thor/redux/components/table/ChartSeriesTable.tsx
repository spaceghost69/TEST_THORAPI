...
import { DataObject, ChartSeries } from '../../../model';
import { useLazyGetChartSeriessQuery, useAddChartSeriesMutation, useUpdateChartSeriesMutation } from '../../services/ChartSeriesService';
...
const ChartSeriesTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetChartSeriessQuery();
  const [addChartSeries] = useAddChartSeriesMutation();
  const [updateChartSeries] = useUpdateChartSeriesMutation();

  const [data, setData] = useState<ChartSeries[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ChartSeries>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ChartSeries | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateChartSeries(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update ChartSeries:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing ChartSeries</Modal.Title>
        </Modal.Header>
        <Modal.Body>
