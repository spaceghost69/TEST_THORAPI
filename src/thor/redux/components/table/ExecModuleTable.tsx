...
import { DataObject, ExecModule } from '../../../model';
import { useLazyGetExecModulesQuery, useAddExecModuleMutation, useUpdateExecModuleMutation } from '../../services/ExecModuleService';
...
const ExecModuleTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetExecModulesQuery();
  const [addExecModule] = useAddExecModuleMutation();
  const [updateExecModule] = useUpdateExecModuleMutation();

  const [data, setData] = useState<ExecModule[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<ExecModule>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<ExecModule | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateExecModule(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update ExecModule:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing ExecModule</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default ExecModuleTable;
