...
import { DataObject, SheetRow } from '../../../model';
import { useLazyGetSheetRowsQuery, useAddSheetRowMutation, useUpdateSheetRowMutation } from '../../services/SheetRowService';
...
const SheetRowTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetSheetRowsQuery();
  const [addSheetRow] = useAddSheetRowMutation();
  const [updateSheetRow] = useUpdateSheetRowMutation();
  ...
  const [copiedRow, setCopiedRow] = useState<SheetRow | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateSheetRow(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update SheetRow:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing SheetRow</Modal.Title>
        </Modal.Header>
        <Modal.Body>
