...
import { DataObject, SalesActivity } from '../../../model';
import { useLazyGetSalesActivitysQuery, useAddSalesActivityMutation, useUpdateSalesActivityMutation } from '../../services/SalesActivityService';
...
const SalesActivityTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetSalesActivitysQuery();
  const [addSalesActivity] = useAddSalesActivityMutation();
  const [updateSalesActivity] = useUpdateSalesActivityMutation();

  const [data, setData] = useState<SalesActivity[]>([]);
  ...
  const [copiedRow, setCopiedRow] = useState<SalesActivity | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateSalesActivity(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update SalesActivity:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing SalesActivity</Modal.Title>
        </Modal.Header>
        <Modal.Body>
