...
import { DataObject, OasServer } from '../../../model';
import { useLazyGetOasServersQuery, useAddOasServerMutation, useUpdateOasServerMutation } from '../../services/OasServerService';
...
const OasServerTable: React.FC = () => {
  const [trigger, { data: newData = [], isFetching }] = useLazyGetOasServersQuery();
  const [addOasServer] = useAddOasServerMutation();
  const [updateOasServer] = useUpdateOasServerMutation();

  const [data, setData] = useState<OasServer[]>([]);
  const [page, setPage] = useState(1);
  const [editRowId, setEditRowId] = useState<string | null>(null);
  const [formData, setFormData] = useState<Partial<OasServer>>({});
  ...
  const [copiedRow, setCopiedRow] = useState<OasServer | null>(null);
  ...
  const handleSave = async (editKey: string) => {
    ...
    try {
      await updateOasServer(updatedItem).unwrap();
      setData((prevData) => prevData.map((item) => (item.id === id ? updatedItem : item)));
    } catch (error) {
      console.error('Failed to update OasServer:', error);
    }
    setEditRowId(null);
    setFormData({});
  };
  ...
  return (
    <div ref={tableRef} className="tableContainer" style={ { overflowY: 'auto', height: '500px' } }>
      ...
      <Modal show={modalVisible} onHide={() => setModalVisible(false)} centered>
        <Modal.Header closeButton>
          <Modal.Title>Editing OasServer</Modal.Title>
        </Modal.Header>
        <Modal.Body>
